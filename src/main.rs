...
fn main() {
    ...
    let prog_str = fs::read_to_string(filename).expect("Unable to read file");
    ...
    let timeout = if filename.starts_with("mttkrp") {
        120
    ...
    // println!("{}", prog);
    let report = run_eqsat(&mut eg, rewrites, 30, timeout, move |egraph| Ok(()));
    let cost_func = SdqlCost { egraph: &eg };
    let extractor = Extractor::<_, SdqlCost>::new(&eg, cost_func);
    ...
    println!(
        ...
        if matches!(report.stop_reason, slotted_egraphs::StopReason::Saturated) {
            "\\yes"
        } else {
            "\\no"
        },
